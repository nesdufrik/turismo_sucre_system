...
defineExpose({
	triggerSearch: () => handleSearch(searchQuery.value),
	clearSearch: () => { searchQuery.value = ''; handleSearch('') }
})
...
watch(
	...
	(val) => {
		...
		const found =
			internalItems.value.find((i) => i[props.valueKey] == val) ||
			props.items.find((i) => i[props.valueKey] == val)
		if (found) {
	...
	<Popover v-model:open="open">
		...
		<PopoverContent class="w-full p-0">
			<Command :disable-filter="mode === 'async'">
				...
				/>
				<CommandList>
					<CommandEmpty v-if="!loading && mode === 'sync'"
						>No se encontraron resultados.</CommandEmpty
					>
					<div
						v-if="!loading && mode === 'async' && internalItems.length === 0"
